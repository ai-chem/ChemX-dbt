...
{% set columns = adapter.get_columns_in_relation(ref('serving_all_data_oxazolidinones')) %}
{% set total_columns = columns | length %}

WITH base AS (
    SELECT * FROM {{ ref('serving_all_data_oxazolidinones') }}
),
...
SELECT
    ...
    COUNT(DISTINCT smiles || '_' || bacteria_name_unified) AS unique_molecule_bacteria_pairs,
